...
localSearchWait = 5 # after how many generations to perform local search
localSearchBestLookup = int(populationSize/100) # number of best solutions to look at for local search
...
def main():
    ...
    for j in range(Generations):
        ...
        if localSearch:
          # Generated by Copilot
          # Check if we should perform local search based on counter
          if (localSeachIterationCount >= 10):
              ...
              for i in range(localSearchBestLookup):
                ...
                #if the best solution is better than the current solution, then replace the current solution with the best solution
                if f_best > Population[ i][1]:
                    Population[i] = (x_best, f_best)
                    print ("Local search improved solution: ", Population[i][1])
                else:
                    print ("Local search did not improve solution: ")
